...
class TempatWisata(Lokasi):
    ...
    def get_info_popup(self) -> str:
        return f"<h4><b>{self.nama}</b></h4><i>Wisata ({self.tipe})</i><br><br>{self.deskripsi}<br><br>Koordinat: ({self.latitude:.4f}, {self.longitude:.4f})"


class Kuliner(Lokasi):
    def __init__(self, nama: str, latitude: float, longitude: float, deskripsi: str):
        super().__init__(nama, latitude, longitude)
        self.menu_andalan = deskripsi # Diasumsikan parameter deskripsi mengisi menu_andalan
